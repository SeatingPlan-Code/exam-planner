...
# Visual styling to eliminate clipping bugs and ensure a clean, compact UI
st.markdown("""
    <style>
...
# Container to display current active rooms
rooms_container = st.container()
updated_rooms = []

with rooms_container:
    for idx, room in enumerate(st.session_state.managed_rooms):
        # Create a clean horizontal row for each room's controls
...
# Update memory state without deleted elements
st.session_state.managed_rooms = updated_rooms

# Button to append an entirely new empty room record row
...
if stud_file:
    if not st.session_state.managed_rooms:
        st.error("❌ Cannot process allocation: You have deleted all available exam rooms! Please add at least one room above.")
    else:
        try:
            ...
            if missing_cols:
                ...
                # Clean and standardize string tokens
